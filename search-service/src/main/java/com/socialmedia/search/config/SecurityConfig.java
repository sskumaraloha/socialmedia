...
/**
 * A pure resource server - search-service never issues tokens, only validates the ones
 * auth-service already issued (see common-library's JwtAuthenticationFilter/JwtValidator).
 * Search results can reveal whether a user/chat/message exists, so every search endpoint
 * requires authentication same as everywhere else in the platform.
 */
@Configuration
@EnableWebSecurity
